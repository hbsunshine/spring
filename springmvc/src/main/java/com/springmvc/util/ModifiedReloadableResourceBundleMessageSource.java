...
/** this is a modified version from the spring's loader, since it has a slight problem of being unable to load
 * anything other than properties and xml files, adding one single line to the loader to simplify the problem
 */
 ...
 */
public class ModifiedReloadableResourceBundleMessageSource extends AbstractMessageSource
implements ResourceLoaderAware {
	...
	private String[] extensions = (String[]) Arrays
			.asList(
					PROPERTIES_SUFFIX,
					XML_SUFFIX).toArray();
	...
	public void setExtensions(final String... extensions) {
		// other than the default, more can be added via this
		if (basenames != null) {
	...
	protected PropertiesHolder getMergedProperties(final Locale locale) {
		synchronized (this.cachedMergedProperties) {
			...
			for (int i = this.basenames.length - 1; i >= 0; i--) {
				final List<?> filenames = calculateAllFilenames(
						this.basenames[i], locale);
				for (int j = filenames.size() - 1; j >= 0; j--) {
					final String filename = (String) filenames.get(j);
					final PropertiesHolder propHolder = getProperties(filename);
	...
	protected List<String> calculateAllFilenames(final String basename, final Locale locale) {
		synchronized (this.cachedFilenames) {
			...
			if (this.fallbackToSystemLocale && !locale.equals(Locale.getDefault())) {
				final List<String> fallbackFilenames = calculateFilenamesForLocale(basename, Locale.getDefault());
		...
		if (!resource.exists()) {
			for (final String extension : extensions) {
				resource = this.resourceLoader
						.getResource(filename + extension);
				if (resource.exists()) {
					break;
				}
			}
	...
	public void clearCacheIncludingAncestors() {
		clearCache();
		if (getParentMessageSource() instanceof ModifiedReloadableResourceBundleMessageSource) {
			((ModifiedReloadableResourceBundleMessageSource) getParentMessageSource()).clearCacheIncludingAncestors();
		}
	}
